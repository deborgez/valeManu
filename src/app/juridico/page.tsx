...
export default async function JuridicoPage() {
  const acordos = await prisma.acordoDistrato.findMany({
    where: { enviadoJuridico: true },
    include: {
      distrato: { include: { processo: { select: { numeroProcesso: true } } } },
        ...
        <table className="w-full text-left text-sm">
          <thead className="bg-slate-50 dark:bg-slate-900 text-slate-600 dark:text-slate-400">
            <tr>
              ...
              <th className="px-4 py-2">Valor do Acordo</th>
            </tr>
          </thead>
          <tbody>
            {acordos.map((a) => (
              ...
              >
                ...
                <td className="px-4 py-2 text-slate-600 dark:text-slate-400">
                  {a.dataEnvioJuridico ? formatData(a.dataEnvioJuridico) : "—"}
                </td>
                <td className="px-4 py-2 text-slate-600 dark:text-slate-400">
                  {a.enviadoJuridicoPor?.nome ?? "—"}
                </td>
                <td className="px-4 py-2 text-slate-600 dark:text-slate-400">
                  {a.motivoJuridico ?? "—"}
                </td>
                <td className="px-4 py-2 text-slate-600 dark:text-slate-400">
                  R$ {formatMoedaExibicao(a.valorFinal)}
                </td>
              </tr>
            ))}
            {acordos.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-6 text-center text-slate-400 dark:text-slate-500">
                  Nenhum acordo enviado para o Jurídico ainda.
                </td>
              </tr>
            )}
          </tbody>
